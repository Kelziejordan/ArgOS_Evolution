...
/**
 * HEALTH MONITOR (The System's Metabolism Checker)
 * 
...
class HealthMonitor {
  ...
  private wireSenses() {
    eventBus.on(NexusEvent.PULSE_TICK, () => {
      ...
      if (this.missedBeats > 0) {
        ...
        this.recoverSubsystem('pipeline', 15);
      }
    });
  ...
  private degradeSubsystem(subsystem: string, impact: number) {
     if (subsystem.includes('brain') || subsystem.includes('risk') || subsystem.includes('VERIFICATION')) {
       ...
       this.subsystemScores.services = Math.max(0, this.subsystemScores.services - impact);
     } else {
       this.subsystemScores.pipeline = Math.max(0, this.subsystemScores.pipeline - impact);
     }
  }

  private recoverSubsystem(category: 'brain' | 'services' | 'pipeline', amount: number) {
  ...
  private assessOverallHealth() {
    const avgScore = (this.subsystemScores.brain + this.subsystemScores.services + this.subsystemScores.pipeline) / 3;
    if (avgScore < 50) {
       console.warn("[HEALTH] System critically degraded. Emitting degraded diagnostics.");
       eventBus.dispatch(NexusEvent.SYSTEM_ALERT, { message: "SYSTEM_HEALTH_CRITICAL: Initiating safety protocols.", level: "CRITICAL" });
    }
  }
  ...
  private checkHeartbeat() {
    ...
    if (delay > this.maxHeartbeatDelay) {
        ...
        this.degradeSubsystem('pipeline', jitterDeduction);
      ...
      this.recoverSubsystem('pipeline', 2);
    }
  }
  ...
  private checkThroughput() {
     ...
     if (delta === 0) {
         ...
         this.degradeSubsystem('pipeline', 3);
     } else {
         // Re-processing signals heals services/nervous channels
         this.recoverSubsystem('pipeline', 1);
         this.recoverSubsystem('services', 1);
     }
